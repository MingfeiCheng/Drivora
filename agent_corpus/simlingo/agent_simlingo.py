...
class LingoAgent(AutonomousAgent):
    """
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        # self.debug_save_path = self.save_path + '/debug_viz' + f'/{self.session}/iter_{self.iter}/{time.strftime("%Y_%m_%d_%H_%M_%S")}'
        ...
        if DEBUG and self.step%5 == 0:
            ...
            if language is not None:
                ...
                if HD_VIZ:
                    ...
                    y_dist = 30
                    y_start = H + 20
                font = ImageFont.truetype(str(Path(__file__).parent / "arial.ttf"), font_size)
                import textwrap
    ...
    def destroy(self, results=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        del self.config
        encoder = None
        if hasattr(self.cfg, "data_module"):
            encoder = getattr(self.cfg.data_module, "encoder", None)
        elif isinstance(self.cfg, dict):
            encoder = self.cfg.get("data_module", {}).get("encoder")

        if encoder == "llavanext":
