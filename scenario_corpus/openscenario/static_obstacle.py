...
from scenario_elements.behavior.atomic import ActorTransformSetter
...
class StaticObstacleConfig(BaseModel):
    ...
    category: Optional[str] = Field("obstacle", description="Vehicle category, e.g., 'car'")
    location: Waypoint = Field(
        ..., description="Behavior configuration for the NPC vehicle"
    )
    
class StaticObstacleScenario(ScenarioTree):
    """Static obstacle scenario tree."""
    ...
    def __init__(
        ...
        self.npc_configs: Dict[str, StaticObstacleConfig] = {cfg.id: cfg for cfg in self.configs}
    ...
    def _initialize_actors(self):
        """Spawn static obstacles at their configured locations."""
        _carla_map = self.map
        for npc_id, npc_cfg in self.npc_configs.items():
            # 1. request new actor
            # -> spawn point of the actor
            npc_spawn_point = npc_cfg.location
