...
class LocalPlanner(object):
    ...
    def _compute_next_waypoints(self, k=1):
        ...
        for _ in range(k):
            ...
            self._waypoints_queue.append((next_waypoint, road_option, 999.0)) # add a default value
    def is_task_finished(self):
        return len(self._waypoints_queue) == 0
    
    def set_global_plan(self, current_plan: List[Waypoint], stop_waypoint_creation=True, clean_queue=True):
        """
        NOTE: when creation, you must generate accurate waypoints
        each item:
        ...
        for i, elem in enumerate(current_plan):
            # TODO: check this line is neccesary?
            if i == 0:
                continue
            ...
            self._waypoints_queue.append(new_elem) # should be a tuple (carla.Waypoint, RoadOption)
    ...
    def run_step(self, debug=False):
        ...
        for waypoint, _, _ in self._waypoints_queue:
        ...
        if len(self._waypoints_queue) == 0:
            ...
            self.target_waypoint, self.target_road_option, assigned_speed = self._waypoints_queue[0]
            # NOTE: this is user specified speed, not the one from the map
            self._target_speed = min(assigned_speed, self._target_speed)
