...
class OpenScenario(BasicScenario):
    ...
    def _setup_scenarios(self):
        ...
        if traffic_light_type is None or traffic_light_type == 'none':
            ...
            try:
                ...
                logger.warning("Skipping scenario '{}' due to setup error: {}".format('TrafficLight', e))
                traceback.print_exc()
    ...
    def _initialize_environment(self):
        if self.config.weather:
            self.world.set_weather(self.config.weather.get_carla_parameters())
    ...
    def _create_test_criteria(self) -> Optional[list]:
        """Create runtime test criteria for each ego vehicle."""
        single_criteria = []
        for ego_id, ego_vehicle in self.ego_vehicles.items():
